...
def get_metrics_reduced(img1, img2):
    ...
    return m1.mean().item(), m2.mean().item(), m3.mean().item(), m4.mean().item(), m5.mean().item()

def get_metrics_full(img1, img2, img3, satellite='QB'):
    m1 = D_lambda(img1, img2)
    m2 = D_s(img1, img2, img3, satellite = satellite)
    m3 = qnr(img1, img2, img3, satellite = satellite)
...
def D_s(img_fake, img_lm, pan, satellite='QB', scale=4, block_size=32, q=1):
    """Spatial distortion
    img_fake, generated HRMS
    img_lm, LRMS
    pan, HRPan"""
...
def qnr(img_fake, img_lm, pan, satellite='QB', scale=4, block_size=32, p=1, q=1, alpha=1, beta=1):
